# Documentation

This directory contains detailed documentation for Auto Cut Image.

## Contents

- [Installation Guide](./installation.md) - How to install auto-cut-image
- [CLI Reference](./cli-reference.md) - Complete CLI command reference
- [API Documentation](./api.md) - Core library API documentation
- [Contributing](../CONTRIBUTING.md) - How to contribute
- [Changelog](../CHANGELOG.md) - Version history

## Quick Links

- [README](../README.md) - Project overview
- [GitHub Issues](https://github.com/Ericwong5021/auto-cut-image/issues) - Report bugs
- [GitHub Discussions](https://github.com/Ericwong5021/auto-cut-image/discussions) - Ask questions
