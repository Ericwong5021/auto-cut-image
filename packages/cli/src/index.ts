...
program
  ...
  .option(
    '--bg-color <color>',
    'Background color for simple mask (hex)',
    '#FFFFFF',
  )
  ...
  .action(
    async (
      ...
      try {
        ...
        const output =
          options.output || input.replace(/(\.[^.]+)$/, '_segmented$1');
        ...
        const resultBuffer = await generateTransparentPng(
          processed.buffer,
          mask,
        );
        await saveImage(
...
program
  ...
  .action(
    async (
      input: string,
      options: { output?: string; bgColor: string; tolerance: string },
    ) => {
      const spinner = ora('Removing background...').start();
      const output =
        options.output || input.replace(/(\.[^.]+)$/, '_transparent$1');
...
program
  ...
  .option(
    '--bg-color <color>',
    'Color to detect as background for mask (hex)',
    '#FFFFFF',
  )
  ...
  .action(
    async (
      ...
      const output =
        options.output || input.replace(/(\.[^.]+)$/, '_bg_replaced$1');
...
program
  ...
  .option(
    '--preset <name>',
    'Use crop preset (square, portrait, landscape, wide, etc.)',
  )
  ...
  .action(
    async (
      ...
      const output =
        options.output || input.replace(/(\.[^.]+)$/, '_cropped$1');
      ...
      try {
        ...
        if (options.smart) {
          ...
          resultBuffer = await cropToAspectRatio(
            processed.buffer,
          ...
          spinner.fail(
            chalk.red(
              'Please specify crop dimensions (--width/--height), --square, --ratio, --preset, or --smart',
            ),
          );
          process.exit(1);
        }
...
program
  ...
  .action(() => {
    ...
    for (const preset of presets) {
      console.log(
        `  ${chalk.green(preset.name.padEnd(20))} ${preset.description} (${preset.ratio}:1)`,
      );
    }
  });
  ...
  .action(
    async (
      ...
      try {
        ...
        spinner.succeed(
          chalk.green(
            `Batch complete: ${progress.completed} succeeded, ${progress.failed} failed`,
          ),
        );
      } catch (error) {
